...
### Exercise
Have a look at the data you read in. Are all variables the correct type?
...
A useful step in using pipes and tidyverse calls is the ability to *select* specific columns. That is, instead of writing `spr_data$RT` we can write:
...
We can then use this column for further calculations, like piping it on to the summary call. This will provide the same result as `summary(spr_data$RT)`
```{r}
spr_data %>% 
  select(RT) %>% 
  summary()
...
#### Try it out - move to extra files?
...
This leads to a lot of repetition. "Mad dog", for example, turns up twice in just our preview. To remedy this, we can use `distinct()`, which only keeps unique rows:
```{r}
animal_corpus %>% 
  separate(col = collocate, 
           into = c("coll", "coll_tag"), 
...
```
As you can see, we can change several variables within one `mutate()`  call.
...
You can also save the new column with the same name, and this will update all the items in that column (see below, where I divide reponse times by 1000, but note that I don't save the output):
```{r}
spr_data %>% 
  mutate(RT = RT / 1000)
```

You can create many new columns at the same time. Here, dropping each new column onto a new line is considered new style (but not necessary!)
