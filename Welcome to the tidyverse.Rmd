...
![Axis labels](img/axes.jpg){width=50%}
Source unknown.
...
```{r}
ggplot(data = spr_complete) +
  aes(x = word_length, y = log(frequency)) +
  geom_point()
```
...
### colour
Let's return to our boxplot of RTs by condition. Here, we can add a color argument to the *geom* call that calls a normal color (*in quotes*). This will make the lines of the boxplot that color.
...
Or (as with color), use it to introduce a new variable.
```{r}
ggplot(data = spr_complete) +
  aes(x = education, fill = gender) +
  geom_bar() #try position="dodge" and/or show.legend=FALSE
```
...
![ggplot masterpiece](img/ggplot2_masterpiece.png){width=50%}

And if you're looking for something new, check out the [R Graph Gallery](https://www.r-graph-gallery.com)
...
Always list and load all packages at the top of your script (in the first code block if using an R-Markdown file)!
...
Similarly, be consistent with capitalisation - our recommendation is to use lowercase for all data and variable names to reduce the chance of misspelling variables.
...
We've also consistently assigned to the dataframe (e.g. spr <- spr %>% some commands) at the beginning of a code block, although it's also possible to do that at the end of the code block, which is not recommended in most cases. 
...
The folder "advanced_pivoting_exercise" contains an exercise that covers a lot of the data wrangling commmands we discussed today and goes into a lot more detail on pivoting and joining.
...
- [Allison Horst's tidyverse introduction](https://allisonhorst.shinyapps.io/edge-of-the-tidyverse)
...
- [Article: Data Vis for Likert Questions](https://lmudge13.github.io/sample_code/likert_graphs.html)
- [Making Vowel Plots in R](https://joeystanley.com/blog/making-vowel-plots-in-r-part-1)
- [Danielle Navarro's Youtube playlist on R-Markdown](https://www.youtube.com/playlist?list=PLRPB0ZzEYegM86awo590bP1MJjQbjdR9E)
- [R Graph Gallery](https://www.r-graph-gallery.com)


![Hopefully, you feel a lot closer to the second picture than when we started!](img/r_first_then.png)
