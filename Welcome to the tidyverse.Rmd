...
- Acceptability judgements (acceptability_judgements.csv)
...
So to get the average RTs -- note the extra step here to drop NAs, this is only because this dataset has NAs we currently want to ignore.
```{r}
spr <- spr %>% 
  drop_na(RT)
...
We have a couple of NAs, lets make sure they're out of the way first. (Remember not to do this without extreme caution on real data, and check the number of rows before and after!)
```{r}
spr_complete <- spr_complete %>% 
  drop_na()
```
