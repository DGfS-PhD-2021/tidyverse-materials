...
We'd also like the information that it stored in the columns starting with "animals" to be summarised in one column called "pet_interest". Here's code that achieves that:
```{r}
animal_survey %>% 
  mutate(pet_interest = (animals_cute + animals_look + animals_pet + animals_have)/4)
```

Here's more elegant code that uses a `select` statement to pick the relevant rows and `rowMeans` to return the averages: 
```{r}
animal_survey %>% 
  mutate(pet_interest = rowMeans(select(., starts_with("animals"))))
```
