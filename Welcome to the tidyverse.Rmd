...
```{r}
<<<<<<< Updated upstream
catdog_spr <- read_csv("data/spr_example.csv")
=======
spr_data <- read_csv("data/dgfs_spr.csv")
>>>>>>> Stashed changes
```
The current dataframe is a self-paced reading experiment where 12 participants read 20 sentences each, plus 3 practice sentences to get them warmed up. Half the sentences were about dogs and half the sentences were about cats. In one condition (A), all sentences were paired with appropriate adjectivcal collocates according to the BNC (lap dog vs. tortiseshell cat), in the other, these were reverse (lap cat vs. tortiseshell dog). All sentences were presented in otherwise natural-sounding sentences. 

## REMOVE? Dataframes and tibbles: What's the difference?
...
```{r}
catdog_spr$participant <- as_factor(catdog_spr$participant)
catdog_spr$item_type <- as_factor(catdog_spr$item_type)
catdog_spr$cond <- as_factor(catdog_spr$cond)
```
...
```{r}
summary(catdog_spr$participant)

summary(catdog_spr$cond)
```
...
This produces the exact same output as `head(catdog_spr)`. Why would this be useful?
...
```{r}
# Equivalent to summary(catdog_spr)
catdog_spr %>% 
  summary()

# Equivalent to colnames(catdog_spr), which returns all column names
colnames(catdog_spr)

catdog_spr %>% 
  colnames() 

# Equivalent to nrow(catdog_spr), which returns the number of rows in the df
nrow(catdog_spr)
...
```
Notice above that I've saved output over the catdog_spr dataframe to make the changes 'permanent'.

There is no output when you simply save the response, but the spr_data dataframe has been permanently updated (within the R session, not in your file system)
...
=======
spr_data %>% 
  #relocate(word_num, .after = word) %>% #col doesnt exist anymore (can add it but wasnt sure how)
>>>>>>> Stashed changes
...
A useful step in using pipes and tidyverse calls is the ability to *select* specific columns. That is, instead of writing `catdog_spr$RT` we can write:
...
We can then use this column for further calculations, like piping it on to the summary call. This will provide the same result as `summary(catdog_spr$RT)`
```{r}
catdog_spr %>% 
  select(RT) %>% 
  summary()
...
- Above: mostly previewing using `select()` for first insights
  - if you look at the catdog_spr dataframe, for example in the Environment panel on the upper-right, the dataframe hasn't changed
...
There is no output here, but the catdog_spr dataframe has been permanently updated (within the R session, not in your file system)

There is also a trick to show a preview and save it to the variable -- wrapping the whole call in parentheses -- use with caution!

```{r}
(catdog_spr <- catdog_spr %>% 
  select(item_type, everything()))
=======
spr_data %>% 
  select(-item_type)
```
...
```{r}
catdog_spr %>% 
  select(-c(word_num, sentence_num))
=======
spr_data <- spr_data %>% 
  select(-c(item_type, X1))
>>>>>>> Stashed changes
...
```
 Some NAs were introduced! To figure out why that's the case, let's use `is.na` within a `filter()` command:
```{r}
animal_corpus %>% 
...
```{r}
# write_tsv(catdog_spr, "catdog_spr_df.txt")

# write_csv(catdog_spr, "catdog_spr_df.csv")
```
