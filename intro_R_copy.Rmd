...
R has a lot of built-in functions, but most of them are basic operations. To do more advanced operations, you'll need add-on *packages*. These packages need to be installed only once but loaded every time you want to use them. Think of this as installing light bulbs when you move into a new flat or house. You only need to do that once (like you only need to install a new R package once), but every evening, you need to switch on the lights in the rooms you use (i.e. load the packages you need).

Use install.packages("package_name") to install packages. We'll start with the `tidyverse` which is actually a collection of several very useful packages.
```{r, eval = FALSE}
...
Then, load the tidyverse with the `library` command:
```{r}
library(tidyverse)
```
...
Be aware: Different system languages can affect csv files. Some languages will separate instead with semicolons (;), especially in languages that use a comma as a decimal separator (ex: 8,35). In those cases, use `read_csv2()` instead of `read_csv()`.

When reading in the data, we also need to assign a name to the data frame using <- again, just like we did when we created variables earlier.
...
Example from R for Data Science (Ch18 - Pipes): https://r4ds.had.co.nz/pipes.html
