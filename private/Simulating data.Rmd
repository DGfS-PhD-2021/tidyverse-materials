...
catdog_spr <- left_join(catdog_spr, corpus_frequencies, by = c("word" = "token")) %>% 
  mutate(frequency = ifelse(is.na(frequency), mean(frequency, na.rm = T), frequency))

 catdog_spr <- catdog_spr %>% 
  mutate(RT = RT - (log(frequency) * 10)) %>%
   select(-frequency) %>% 
   mutate(RT = ifelse(RT < 0, abs(RT), RT)) %>% 
   mutate(RT = round(RT))
...
write_csv(animal_survey, "data/animal_survey.csv")
```
...
lexicaldec <- tibble(participant = rep(c("AD4", "BH5", "HG8", "LK9", "YU0", "SD2", "RT4", "NG2", "PO9", "JL0", "WS8", "RN4", "XA7", "FG5", "RW1", "KL9", "BN0", "FN3", "YU7", "AQ2"), each = 44),
       ...
       RT = round(abs(RT))) %>% 
  mutate(item_id = ifelse(item_id %in% c("item_41", "item_42", "item_43", "item_44"), "practice", item_id))
...
participant_lex <- tibble(participant = c("AD4", "BH5", "HG8", "LK9", "YU0", "SD2", "RT4", "NG2", "PO9", "JL0", "WS8", "RN4", "XA7", "FG5", "RW1", "KL9", "BN0", "FN3", "YU7", "AQ2"),
       ...
       fave_1 = c("recipes", "TidyVerse", "ggPlot",  "dPlyr", "dPlyr", "ggPlot", "dPlyr", "ggPlot", "TidyVerse", "ggPlot", "TidyVerse", "ggPlot", "TidyVerse", "ggPlot", "dPlyr", "dPlyr", "ggPlot", "ggPlot", "TidyVerse", "TidyVerse"),
       fave_2 = c("stringR", "recipes", "viridis", "CowPlot", "reticulate", "stringR", "recipes", "viridis", "CowPlot", "reticulate", "stringR", "recipes", "viridis", "CowPlot", "reticulate", "stringR", "recipes", "viridis", "CowPlot", "reticulate"),
       fave_3 = c("WordCloud2", "viridis", "stringR", "recipes", "CowPlot", "WordCloud2", "viridis", "stringR", "recipes", "CowPlot", "WordCloud2", "viridis", "stringR", "recipes", "CowPlot", "WordCloud2", "viridis", "stringR", "recipes", "CowPlot")
       )
...
Beke's original data:
```{r}
wh_split <- read_csv("archive/wh-split_output_Julia.xls")

wh_split <- wh_split %>% 
  mutate(harmonicity = as.numeric(harmonicity))

wh_wh <- wh_split %>% 
  filter(str_detect(word, "WH"))

wh_w <- wh_split %>% 
  filter(str_detect(word, "WH", negate = TRUE)) %>% 
  filter(word %in% c("WAS", "WE", "WERE", "ONE", "WITH", "WELL", "WAY", "WENT", "WOULD", "ALWAYS"))
...
wh_small <- bind_rows(wh_w, wh_wh)

wh_small <- wh_small %>% 
  select(file, word, fol_sound, harmonicity) %>% 
  mutate(across(where(is.character), as.factor)) %>% 
  relocate(file, fol_sound)
...
wh_final <- wh_syn$syn %>% 
  mutate(duration = rnorm(668, mean=0.5, sd= 0.2),
         time = rnorm(668, mean=200, sd=100)) %>% 
  mutate(duration = abs(duration),
         time = abs(time),
         file = wh_small$file) %>% 
  mutate(harmonicity = if_else(file %in% c("59_1", "78_1", "80_1"),
                               harmonicity + 12, harmonicity)) %>% 
  replace_na(list(harmonicity = "--undefined--")) %>% 
  relocate(file, word, fol_sound)

write_tsv(wh_final, "data/wh_split.txt")
```
